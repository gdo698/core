    ...
    public Page<TotalStockDTO> findBranchStockSummary(
        ...
        if (storeId == null) {
            ...
            jpql.append("SELECT new com.core.erp.dto.TotalStockDTO(")
                    ...
                    .append("(SELECT COALESCE(SUM(wse.quantity), 0) FROM WarehouseStockEntity wse WHERE wse.product = p AND wse.store = s), ")
                    .append("SUM(ss.quantity) + (SELECT COALESCE(SUM(wse.quantity), 0) FROM WarehouseStockEntity wse WHERE wse.product = p AND wse.store = s), ")
